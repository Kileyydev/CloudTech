...
const CACHE_KEY = `audio_accessories_cache`;
...
const AudioAccessoriesSection = () => {
  ...
  const API_BASE = `${process.env.NEXT_PUBLIC_API_BASE_URL}/products/?category=${categorySlug}`;
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        console.error('Error fetching audio accessories:', err);
        if (mounted.current) {
          setSnackbar({ open: true, message: 'Failed to load audio accessories', severity: 'error' });
          setLoading(false);
        }
      }
    };
  ...
  const handleAddToCart = (product: Product) => {
    ...
    if (cartItem && cartItem.quantity >= product.stock)
      return showSnackbar(`Only ${product.stock} items in stock`, 'error');
  ...
  const getCartItemCount = () => Object.values(cart).reduce((t, i) => t + (i.quantity || 0), 0);

  // 🩶 Skeleton Loader (same as ProductSection)
  ...
  const renderCard = (product: Product) => {
    ...
    return (
      ...
      >
        ...
        {/* 📦 Card Content */}
        <CardContent
          sx={{ flexGrow: 1, p: 1.5, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: 180 }}
        >
          <Box>
            ...
            <Typography
              variant="body2"
              sx={{ color: '#666', fontSize: '0.85rem', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', mt: 0.5 }}
            >
