...
const CACHE_KEY = 'audio_accessories_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (!res.ok) throw new Error('Failed to fetch audio products');
        ...
        if (list.length === 0) throw new Error('No audio accessories found');
      ...
      >
        ...
        <CardContent sx={{ flexGrow: 1, p: 1.5, pb: '12px !important', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box>
            ...
            {/* Placeholder rating – feel free to make it dynamic later */}
        ...
        >
          <Box sx={{ display: 'flex', gap: 2 }}>
            {loading
              ...
              ? <Typography sx={{ py: 5, pl: 2 }}>No audio accessories available right now.</Typography>
