...
const CACHE_KEY = 'gaming_accessories_cache_v3';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const filters = [
          'category__slug=gaming',
          'categories__slug=gaming',
        ...
        for (const filter of filters) {
          const slug = 'gaming'; // or use the correct slug for your category
const res = await fetch(`${API_BASE_URL}/products/?categories__slug=${slug}`, {
  cache: 'no-store',
        ...
        if (finalProducts.length === 0)
          throw new Error('No gaming accessories found');
