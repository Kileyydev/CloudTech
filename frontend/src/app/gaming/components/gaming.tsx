...
const CACHE_KEY = 'gaming_accessories_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (list.length === 0) throw new Error('No gaming accessories found');
        ...
        console.error('Gaming fetch failed:', err);
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load gaming accessories',
          severity: 'error',
        });
      } finally {
        setLoading(false);
      ...
      >
        ...
        <CardContent sx={{ flexGrow: 1, p: 1.5, pb: '12px !important', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box>
            ...
            {/* Placeholder rating – make dynamic later if needed */}
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                ? <Typography sx={{ py: 5, pl: 2 }}>No gaming accessories available right now.</Typography>
        ...
        <Box sx={{ display: { xs: 'none', lg: 'block' } }}>
          {loading ? (
            ...
            <Typography sx={{ textAlign: 'center', py: 5 }}>
              No gaming accessories available right now.
            </Typography>
          ) : (
            <Box
              sx={{
