...
const CACHE_KEY = 'mobile_accessories_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const slug = 'mobile-accessories';
        const res = await fetch(`${API_BASE_URL}/products/?categories__slug=${slug}`, {
          cache: 'no-store',
        });

        if (!res.ok) throw new Error('Failed to fetch mobile accessories');
        ...
        if (list.length === 0) throw new Error('No mobile accessories found');
        ...
        console.error('Mobile fetch failed:', err);
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load mobile accessories',
          severity: 'error',
        });
      } finally {
        setLoading(false);
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                ? <Typography sx={{ py: 5, pl: 2 }}>No mobile accessories available right now.</Typography>
        ...
        <Box sx={{ display: { xs: 'none', lg: 'block' } }}>
          {loading ? (
            ...
            <Typography sx={{ textAlign: 'center', py: 5 }}>
              No mobile accessories available right now.
            </Typography>
          ) : (
            <Box
              sx={{
