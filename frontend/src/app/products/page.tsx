...
const CACHE_KEY = 'featured_cache_v3';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const filters = [
          'category__slug=popularsection',
          'categories__slug=popularsection',
          'category_slug=popularsection',
          'category=popularsection',
        ];

        let finalProducts: ProductT[] = [];
        for (const filter of filters) {
         const slug = 'popularsection'; // or use the correct slug for your category
const res = await fetch(`${API_BASE_URL}/products/?categories__slug=${slug}`, {
  cache: 'no-store',
        ...
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load featured products',
          severity: 'error',
        });
      } finally {
        setLoading(false);
