...
interface ProductT {
  ...
  images?: ProductImage[];  // Added for gallery
  brand?: { name: string };
  colors?: { value: string }[];
...
const CACHE_KEY = 'featured_cache_v4';
const CACHE_DURATION = 1000 * 60 * 10; // 10 minutes
const INITIAL_LOAD = 32;
const LOAD_MORE_INCREMENT = 32;
...
const FeaturedSection = () => {
  ...
  const renderCard = (product: ProductT) => {
    ...
    return (
      ...
      >
        ...
        {/* Gallery preview at bottom - same as Deals */}
  ...
  const displayedProducts = inStockProducts.slice(0, displayedCount);
  const hasMore = displayedCount < inStockProducts.length;
      ...
      <Box sx={{ bgcolor: '#fdfdfd', py: { xs: 3, lg: 4 }, px: { xs: 2, lg: 3 } }}>
          ...
          <Box sx={{ overflowX: 'auto', display: 'flex', pb: 2, scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                : displayedProducts.map(product => <Box key={product.id}>{renderCard(product)}</Box>)
              }
            </Box>
          </Box>
        </Box>
        ...
        {hasMore && !loading && (
          <Box sx={{ mt: 5, textAlign: 'center' }}>
            ...
            <Divider sx={{ bgcolor: '#000', height: 2, mt: 4 }} />
          </Box>
        )}
      </Box>
