...
import {
  ...
  CircularProgress, MenuItem, Select, FormControl, Stack, Snackbar, Alert,
  Table, TableHead, TableRow, TableCell, TableBody, IconButton, Tooltip,
} from "@mui/material";
import { CheckCircle, Inventory, AccessTime, LocalShipping, DoneAll, Refresh } from "@mui/icons-material";
...
// CORRECT ENDPOINT: /purchases/ (from urls.py: path('', ...))
...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = async (url: string = ORDERS_URL) => {
    if (!token) {
      console.error("NO TOKEN: Admin not logged in. Key 'access' missing in localStorage.");
    ...
    try {
      ...
      if (Array.isArray(raw)) list = raw;
        ...
        console.warn("Unexpected payload format:", raw);
      }
          ...
          <CardContent>
                      ...
                      <TableRow key={order.id} hover>
                        ...
                        <TableCell>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {order.phone}<br />{order.city}, {order.address}
                          </Typography>
                        </TableCell>
                            ...
                            >
                              {STATUS_STEPS.map(s => (
                                <MenuItem key={s.key} value={s.key}>
                                  <Chip label={s.label} icon={s.icon} size="small"
                                    sx={{ bgcolor: `${s.color}20`, color: s.color }}
                                  />
                                </MenuItem>
                              ))}
                            </Select>
            ...
            {nextUrl && (
              <Box textAlign="center" mt={3}>
                <Button variant="outlined" onClick={() => fetchOrders(nextUrl)} disabled={loading}>
