...
const CACHE_KEY = 'powerbank_accessories_cache_v4';
const CACHE_DURATION = 1000 * 60 * 10; // 10 mins

const PowerbankAccessoriesSection = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const slug = 'powerbank';
        const res = await fetch(`${API_BASE_URL}/products/?categories__slug=${slug}`, {
          cache: 'no-store',
        });

        if (!res.ok) throw new Error('Failed to fetch powerbank products');
        ...
        if (list.length === 0) throw new Error('No powerbank products found');
        ...
        console.error('Powerbank fetch failed:', err);
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load powerbank accessories',
          severity: 'error',
        });
      } finally {
        setLoading(false);
      ...
      >
        ...
        <CardContent sx={{ flexGrow: 1, p: 1.5, pb: '12px !important', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box>
            ...
            {/* Placeholder rating – can be made dynamic later */}
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                ? <Typography sx={{ py: 5, pl: 2 }}>No powerbank accessories available right now.</Typography>
        ...
        <Box sx={{ display: { xs: 'none', lg: 'block' } }}>
          {loading ? (
            ...
            <Typography sx={{ textAlign: 'center', py: 5 }}>
              No powerbank accessories available right now.
            </Typography>
          ) : (
            <Box
              sx={{
...
export default PowerbankAccessoriesSection;