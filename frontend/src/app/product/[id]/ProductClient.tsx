...
import { Favorite, ShoppingCart, ArrowBack, Business } from '@mui/icons-material';
...
const ProductClient = ({ initialProduct }: { initialProduct: ProductT }) => {
  const theme = useTheme();
  const router = useRouter();
  const { cart, addToCart } = useCart();

  const [product] = useState<ProductT>(initialProduct);
  ...
  // Initialize defaults
  useEffect(() => {
    if (product.ram_options?.length) setSelectedRam(product.ram_options[0].value);
    if (product.storage_options?.length) setSelectedStorage(product.storage_options[0].value);
    if (product.colors?.length) setSelectedColor(product.colors[0].value);
    else if (product.color_options?.length) setSelectedColor(product.color_options[0].value);

    setMainImage(getImageUrl(product.cover_image));
    updatePrice(product, '', '', '');
  }, [product]);
  ...
  // Update price
  useEffect(() => {
    updatePrice(product, selectedRam, selectedStorage, selectedColor);
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: { xs: 2, md: 4 }, bgcolor: '#fdfdfd' }}>
      ...
      <Box sx={{ display: 'flex', gap: { xs: 2, md: 4 }, flexDirection: { xs: 'column', lg: 'row' } }}>
        ...
        <Box sx={{ flex: '0 0 500px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ bgcolor: '#fff', border: '1px solid #eee', borderRadius: 2, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}>
            <img src={mainImage} alt={product.title} style={{ width: '100%', height: 'auto', maxHeight: 500, objectFit: 'contain' }} />
          </Box>
                ...
                <Tooltip key={i} title="Click to enlarge">
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 1, overflow: 'hidden', cursor: 'pointer', flexShrink: 0,
                      transition: 'all 0.2s', '&:hover': { borderColor: '#e91e63' },
                    }}
                  >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          {product.brand && (
            <Chip icon={<Business sx={{ fontSize: 16 }} />} label={product.brand.name}
          ...
          <Stack spacing={2} sx={{ mb: 4 }}>
            {ramOptions.length > 0 && (
              <FormControl fullWidth>
                ...
                <Select value={selectedRam} onChange={e => setSelectedRam(e.target.value)} label="RAM">
                  {ramOptions.map(opt => <MenuItem key={opt.id} value={opt.value}>{opt.value}</MenuItem>)}
                </Select>
              </FormControl>
            )}
            {storageOptions.length > 0 && (
              <FormControl fullWidth>
                ...
                <Select value={selectedStorage} onChange={e => setSelectedStorage(e.target.value)} label="Storage">
                  {storageOptions.map(opt => <MenuItem key={opt.id} value={opt.value}>{opt.value}</MenuItem>)}
                </Select>
              </FormControl>
            )}
            {colorOptions.length > 0 && (
              <FormControl fullWidth>
                ...
                <Select value={selectedColor} onChange={e => setSelectedColor(e.target.value)} label="Color"
                  renderValue={sel => (
          ...
          <Button fullWidth size="large" variant="contained" startIcon={<ShoppingCart />} onClick={handleAddToCart}
            sx={{ bgcolor: '#e91e63', color: '#fff', fontWeight: 700, textTransform: 'none', py: 1.8, fontSize: '1.1rem', mb: 3, '&:hover': { bgcolor: '#c2185b' } }}
          >
            Add to Cart
          </Button>

          <Button fullWidth variant="outlined" startIcon={<Favorite sx={{ color: isInWishlist ? '#e91e63' : '#bbb' }} />} onClick={handleWishlist}
            sx={{ borderColor: '#ddd', color: '#1a1a1a', textTransform: 'none', fontWeight: 600, mb: 4 }}
          >
          ...
          <Typography variant="h6" sx={{ fontWeight: 700, mb: 2, color: '#1a1a1a' }}>Description</Typography>
