...
const CACHE_KEY = 'projectors_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const slug = 'projectors';
        const res = await fetch(`${API_BASE_URL}/products/?categories__slug=${slug}`, {
          cache: 'no-store',
        });

        if (!res.ok) throw new Error('Failed to fetch projectors');
        ...
        if (list.length === 0) throw new Error('No projectors found');
        ...
        console.error('Projectors fetch failed:', err);
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load projectors',
          severity: 'error',
        });
      } finally {
        setLoading(false);
        ...
        >
          <Box sx={{ display: 'flex', gap: 2 }}>
            {loading
              ...
              ? <Typography sx={{ py: 5, pl: 2 }}>No projectors available right now.</Typography>
