...
const CACHE_KEY = 'apple_accessories_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (list.length === 0) throw new Error('No Apple products found');
      ...
      >
        ...
        <CardContent sx={{ flexGrow: 1, p: 1.5, pb: '12px !important', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box>
            ...
            {/* Fake rating – replace with real later if needed */}
        ...
        >
          <Box sx={{ display: 'flex', gap: 2 }}>
            {loading
              ...
              ? <Typography sx={{ py: 5, pl: 2 }}>No Apple accessories available right now.</Typography>
