...
const CACHE_KEY = 'storage_accessories_products_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (!res.ok) throw new Error('Failed to fetch storage products');
        ...
        if (list.length === 0) throw new Error('No storage products found');
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                ? <Typography sx={{ py: 5, pl: 2 }}>No storage accessories available right now.</Typography>
