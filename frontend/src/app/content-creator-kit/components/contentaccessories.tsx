...
const CACHE_KEY = 'content_creator_kit_cache';
const CACHE_TIME = 15 * 60 * 1000; // 15 minutes
const categorySlug = 'content-creator-kit';
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        console.error('Error fetching content creator kits:', err);
        if (mounted.current) {
          setSnackbar({ open: true, message: 'Failed to load content creator kits', severity: 'error' });
          setLoading(false);
        }
      }
    };
  ...
  const renderCard = (product: ProductT) => {
    ...
    const imageSrc =
      images[currentIndex]?.startsWith('http')
        ? images[currentIndex]
        : `https://cloudtech-c4ft.onrender.com${images[currentIndex]}` || '/images/fallback.jpg';
...
export default ContentAccessoriesSection;