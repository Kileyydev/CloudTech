...
const CACHE_KEY = 'smartphone_products_cache_v4';
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (list.length === 0) throw new Error('No smartphones found');
        ...
        setSnackbar({
          open: true,
          message: err.message || 'Failed to load smartphones',
          severity: 'error',
        });
      } finally {
        setLoading(false);
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              {loading
                ...
                ? <Typography sx={{ py: 5, pl: 2 }}>No smartphones available right now.</Typography>
